        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6" data-testid="grid-best-sellers">
          {featured.slice(0, 8).map((product: FeaturedProduct) => (
            <div
              ...
              role="button"
              tabIndex={0}
              className="group flex flex-col bg-card border rounded-xl overflow-hidden hover:shadow-lg hover:border-primary/20 transition-all duration-300 text-left cursor-pointer"
              data-testid={`card-best-seller-${product.id}`}
              onClick={() => setLocation(`/product?id=${product.id}`)}
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ") {
                  e.preventDefault();
                  setLocation(`/product?id=${product.id}`);
                }
              }}
            >
